...
IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name='FK_Departments_Instructors_Head')
ALTER TABLE dbo.Departments
    ADD CONSTRAINT FK_Departments_Instructors_Head
        FOREIGN KEY (DepartmentHeadId) REFERENCES dbo.Instructors(Id) ON DELETE SET NULL;

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name='UX_Departments_DepartmentHeadId')
CREATE UNIQUE INDEX UX_Departments_DepartmentHeadId ON dbo.Departments(DepartmentHeadId)
...
BEGIN
ALTER TABLE dbo.Courses ALTER COLUMN Credits DECIMAL(5,2) NOT NULL;
END